...
class _EmaSettingsScreenState extends State<EmaSettingsScreen> {
  ...
  void initState() {
    super.initState();
    final config = context.read<EmaIndicatorService>().configFor(
      widget.dataType,
    );
  ...
  Future<void> _save() async {
    ...
    try {
      await context.read<EmaIndicatorService>().updateConfigFor(
        dataType: widget.dataType,
        newConfig: EmaConfig(shortPeriod: _shortPeriod, longPeriod: _longPeriod),
      );

      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('$_scopeLabel EMA参数已保存')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
  ...
  Future<void> _resetDefaults() async {
    final defaults = EmaIndicatorService.defaultConfigFor(widget.dataType);
    setState(() {
      _shortPeriod = defaults.shortPeriod;
      _longPeriod = defaults.longPeriod;
    });
    await _save();
  }
  ...
  Future<void> _recompute() async {
    ...
    final progressNotifier =
        ValueNotifier<({int current, int total, String stage})>((
          current: 0,
          total: 1,
          stage: '准备重算$_scopeLabel EMA...',
        ));
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (_) => _EmaRecomputeProgressDialog(
    ...
    try {
      ...
      await context.read<EmaIndicatorService>().prewarmFromRepository(
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('$_scopeLabel EMA重算完成')));
    } catch (e) {
      if (!mounted) return;
      Navigator.of(context).pop();
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('$_scopeLabel EMA重算失败: $e')));
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('${_scopeLabel}EMA设置')),
      body: ListView(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'EMA 使用指数移动平均公式，短线默认周期 9，长线默认周期 21。',
          ...
          _IntParameterCard(
            title: '短周期 (Short Period)',
            hint: '短期均线，默认 9，越小越敏感',
            value: _shortPeriod,
            min: 2,
            max: 60,
            divisions: 58,
            onChanged: (value) => setState(() => _shortPeriod = value),
          ),
          const SizedBox(height: 12),
          _IntParameterCard(
            title: '长周期 (Long Period)',
            hint: '长期均线，默认 21，须大于短周期',
            value: _longPeriod,
            min: 3,
            max: 200,
            divisions: 197,
          ...
          SizedBox(
            ...
            child: FilledButton.tonalIcon(
              key: ValueKey('ema_recompute_${widget.dataType.name}'),
              ...
              label: Text(_isRecomputing ? '重算中...' : '重算$_scopeLabel EMA'),
            ),
          ),
        ],
      ),
...
class _EmaRecomputeProgressDialog extends StatelessWidget {
  const _EmaRecomputeProgressDialog({
