...
class _ProfilingMacdCacheStore extends MacdCacheStore {
  _ProfilingMacdCacheStore({required super.storage});
...
class _ProfilingMacdIndicatorService extends MacdIndicatorService {
  _ProfilingMacdIndicatorService({
...
void main() {
  final runBench = Platform.environment['RUN_REAL_WEEKLY_MACD_BENCH'] == '1';
  final stockLimit =
      int.tryParse(
        Platform.environment['WEEKLY_MACD_BENCH_STOCK_LIMIT'] ?? '',
      ) ??
      200;
  final poolSize =
      int.tryParse(Platform.environment['WEEKLY_MACD_BENCH_POOL_SIZE'] ?? '') ??
      12;
  final rangeDays =
      int.tryParse(
        Platform.environment['WEEKLY_MACD_BENCH_RANGE_DAYS'] ?? '',
      ) ??
      760;
  final sweepRaw =
      Platform.environment['WEEKLY_MACD_BENCH_SWEEP'] ?? '40x6,80x8,120x8';
  final sweepConfigs = _parseSweepConfigs(sweepRaw);

  group('Weekly MACD Recompute Benchmark', () {
    ...
    late _ProfilingMacdCacheStore cacheStore;
    ...
    setUp(() async {
      ...
      cacheStore = _ProfilingMacdCacheStore(storage: fileStorage);
    });
    ...
    test(
      'benchmarks weekly MACD recompute latency and throughput sweep',
      () async {
        ...
        // ignore: avoid_print
        print(
          '[WEEKLY_MACD_BENCH][seed] stocks=${stockCodes.length} '
        ...
        for (final config in sweepConfigs) {
          ...
          final service = _ProfilingMacdIndicatorService(
          ...
          // ignore: avoid_print
          print(
            '[WEEKLY_MACD_BENCH][run] fetchBatch=${config.fetchBatchSize} '
            'persistConcurrency=${config.persistConcurrency} '
        ...
        // ignore: avoid_print
        print(
          '[WEEKLY_MACD_BENCH][best] fetchBatch=${best['fetchBatchSize']} '
      ...
      skip: runBench
          ? false
          : 'Set RUN_REAL_WEEKLY_MACD_BENCH=1 to run benchmark',
...
List<_SweepConfig> _parseSweepConfigs(String raw) {
  final tokens = raw
      ...
      .toList(growable: false);
  if (tokens.isEmpty) {
    throw ArgumentError('WEEKLY_MACD_BENCH_SWEEP cannot be empty');
  }
