...
STOCK_LIMIT="${WEEKLY_ADX_BENCH_STOCK_LIMIT:-200}"
POOL_SIZE="${WEEKLY_ADX_BENCH_POOL_SIZE:-12}"
RANGE_DAYS="${WEEKLY_ADX_BENCH_RANGE_DAYS:-760}"
SWEEP="${WEEKLY_ADX_BENCH_SWEEP:-40x6,80x8,120x8}"

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
LOG_FILE="/tmp/weekly_adx_recompute_bench_${TIMESTAMP}.log"
SUMMARY_FILE="/tmp/weekly_adx_recompute_bench_${TIMESTAMP}.tsv"

echo "[INFO] Running weekly ADX recompute sweep on device: ${DEVICE}"
echo "[INFO] stockLimit=${STOCK_LIMIT}, poolSize=${POOL_SIZE}, rangeDays=${RANGE_DAYS}, sweep=${SWEEP}"

RUN_REAL_WEEKLY_ADX_BENCH=1 \
WEEKLY_ADX_BENCH_STOCK_LIMIT="${STOCK_LIMIT}" \
WEEKLY_ADX_BENCH_POOL_SIZE="${POOL_SIZE}" \
WEEKLY_ADX_BENCH_RANGE_DAYS="${RANGE_DAYS}" \
WEEKLY_ADX_BENCH_SWEEP="${SWEEP}" \
flutter test test/integration/weekly_adx_recompute_benchmark_test.dart \
  -d "${DEVICE}" \
...
rg '\[WEEKLY_ADX_BENCH\]\[run\]' "${LOG_FILE}" | while read -r line; do
...
print('\n=== Weekly ADX Recompute Sweep Summary ===')
